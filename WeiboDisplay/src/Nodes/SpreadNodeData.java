...
public class SpreadNodeData implements Comparable<SpreadNodeData> {
	 ...
	 * @param post_time
	 *            该节点微博发送的时间
	 * @param ArrayList<String> repost_link 转发链，最小大小为1
	 * @param repost_count
	 *            该节点的转发总数
	 * @author Jayvee
	 */
	...
	public void addChild(String child_id) {
		...
		this.setRepost_count(this.getRepost_count()+1);
	}
	...
	public int compareTo(SpreadNodeData o) {
		...
		long oposttime = 0;
		try {
			time = WeiboSpreadUtils.parseTime(this.post_time);
			oposttime=WeiboSpreadUtils.parseTime(o.post_time);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return (int) (time - oposttime);// 以发送时间升序排序
	}
