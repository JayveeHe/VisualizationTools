...
public class MySurfaceView extends SurfaceView implements
	...
	private boolean isByMap = false;
	String screenshot_name = "";

	Map<String, IDrawableNode> info_map;
	Map<String, WeiboNodeData> map;
	...
	public MySurfaceView(Context context, Map<String, IDrawableNode> map) {
		super(context);
		isByMap = true;
		this.info_map = map;
		holder = this.getHolder();
	...
	public MySurfaceView(Context context, File gexffile) {
		...
		map = GexfUtils.gexfDecoder(gexffile);
		holder = this.getHolder();
		holder.addCallback(this);
		// pp.setColor(Color.RED);
		setFocusable(true);
	...
	public MySurfaceView(Context context, InputStream ipstrm) {
		...
		// TODO Auto-generated constructor stub
		this.ipstrm = ipstrm;
		map = GexfUtils.gexfDecoder(ipstrm);
		holder = this.getHolder();
		holder.addCallback(this);
		myThread = new MyThread(holder);
		// pp.setColor(Color.RED);
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		logicManager = new LogicManager(iViewHeight, iViewWidth, iViewRow,
				iViewCol);
		ArrayList<NodeDomainLogic> logiclist = null;
		if (!isByMap) {
			logiclist = NodeDomainLogic
					.creatDomainLogicByMap(map, logicManager);
		} else {
			logiclist = NodeDomainLogic.creatDomainLogicByInfoMap(info_map,
					logicManager);
		}
		logicManager.NamesMap = new HashMap<String, NodeDomainLogic>();
		for (int i = 0; i < logiclist.size(); i++) {
			NodeDomainLogic logic = logiclist.get(i);
			logicManager.addDomainLogic(logic);
			logicManager.NamesMap.put(logic.getData().key, logic);
		}
		// logicManager.onScaleLocated(logicManager.getNodesMap()., logicY,
		// viewX, viewY);
		myThread = new MyThread(holder);
				...
				try {
					synchronized (holder) {
						...
						if (!isByMap) {
							...
							canvas.drawText(
									"共绘制" + logicManager.NodesMap.size() + "个点",
									0, 20, p);
						}
