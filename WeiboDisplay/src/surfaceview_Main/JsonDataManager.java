...
public class JsonDataManager {
	...
	public Map<String, WeiboNodeData> ReadWeiboNodesFromStr(String jstr) {
		JSONTokener jsonTokener = new JSONTokener(jstr);
//		ArrayList<WeiboData> datalist = new ArrayList<WeiboData>();
		Map<String, WeiboNodeData> wbdtmap = new HashMap<String, WeiboNodeData>();
		try {
			...
			for (int i = 0; i < Nodes.length(); i++) {
				...
				WeiboNodeData wd = new WeiboNodeData(x, y, z, group, key,String.valueOf(i));
//				datalist.add(wd);
			...
			for (int i = 0; i < Paths.length(); i++) {
				...
				WeiboNodeData parent = wbdtmap.get(Node1);
				WeiboNodeData child = wbdtmap.get(Node2);
