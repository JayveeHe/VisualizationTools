...
import ForceDirectDemo.ForceDirectMethod;
...
public class LogicManagerDemo {
	// 相关的变量设置
	public final static String DEBUG_TAG = "LogicManager";
	public DrawingMatrix[][] drawingMatrixs = null;
	Map<Long, NodeDomainLogicDemo> NodesMap = null;
	ArrayList<NodeDomainLogicDemo> AtomNodeLogics = null;
	// 相关参数
	...
	public float fScaleRate = 1f;// 缩放比例
	 ...
	 * @param iViewCol
	 */
	public LogicManagerDemo(int iViewHeight, int iViewWidth, int iViewRow,
			int iViewCol) {
		...
		NodesMap = new HashMap<Long, NodeDomainLogicDemo>();
		AtomNodeLogics = new ArrayList<NodeDomainLogicDemo>();
	...
	public void FDA() {
		...
		for (NodeDomainLogicDemo logic_i : NodesMap.values()) {
			// 依次计算逻辑点j对逻辑点i的坐标影响
			float xi = logic_i.getData().getCurX();
			float yi = logic_i.getData().getCurY();
			for (NodeDomainLogicDemo logic_j : NodesMap.values()) {// 进行全节点的遍历
				if (logic_i != logic_j) {
	 ...
	 * @return 待获取的逻辑
	 */
	public NodeDomainLogicDemo getDomainLogic(long ID) {
		return NodesMap.get(ID);
	}

	public void addDomainLogic(NodeDomainLogicDemo domainLogic) {
		try {
		...
		NodeDomainDataDemo data = domainLogic.getData();
//		DrawingMatrix dm = transXY2CR(data.getCurX(), data.getCurY());
	 ...
	 * @param ChildLogic
	 */
	public void addChildDomainLogic(NodeDomainLogicDemo ParentLogic,
			NodeDomainLogicDemo ChildLogic) {
		// 首先修改母节点的距离信息,只有在母节点第一次添加子节点时调用
		if (!ParentLogic.isExpand()) {
			ParentLogic.getData().onModify(NodeDomainDataDemo.MODIFY_LEN,
		...
		try {
			...
			ChildLogic.getData().onModify(NodeDomainDataDemo.MODIFY_LEN,
	...
	public void removeDomainLogic(NodeDomainLogicDemo domainLogic) {
		try {
			NodeDomainDataDemo data = domainLogic.getData();
	 ...
	 *            母节点的逻辑对象
	 */
	public void onUpdate(NodeDomainLogicDemo parentNodeLogic) {
	}
	...
	public void onOverallUpdate() {
		for (NodeDomainLogicDemo logic : NodesMap.values()) {
			NodeDomainDataDemo data = logic.getData();
			data.onModify(NodeDomainDataDemo.MODIFY_X, data.getCurX()
					+ data.Xdiffer, this);
			data.onModify(NodeDomainDataDemo.MODIFY_Y, data.getCurY()
					+ data.Ydiffer, this);
			data.Xdiffer = 0;
	...
	public Map<Long, NodeDomainLogicDemo> getNodesMap() {
		return NodesMap;
	}
