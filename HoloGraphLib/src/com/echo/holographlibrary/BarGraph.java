...
public class BarGraph extends View {
	...
	public BarGraph(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mOrientation = a.getInt(R.styleable.BarGraph_orientation,
	...
	public void onDraw(Canvas canvas) {
		...
		float barWidth = (getWidth() - (padding * 2) * mBars.size())
				/ mBars.size();
		...
		for (final Bar bar : mBars) {
			...
			// 绘制平均高度
			int ave_top = (int) (getHeight() - bottomPadding - (usableHeight * (bar
					.getAveValue() / maxValue)));
			Paint ave_p = new Paint();
			ave_p.setColor(Color.parseColor("#336666"));
			ave_p.setStrokeWidth(2 * resources.getDisplayMetrics().density);
			ave_p.setAntiAlias(true);
			canvas.drawLine(left, ave_top, right, ave_top, ave_p);
			Log.d("绘制平均高度", bar.getName());
			...
			if (mShowAxis) {
				...
				while (right - left + (padding * LABEL_PADDING_MULTIPLIER) < textWidth) {
					// decrease text size to fit and not overlap with other
					// labels.
			...
			if (mShowBarText) {
				...
				this.mPaint
						.getTextBounds(bar.getValueString(), 0, 1, mTextRect);

				int boundLeft = (int) (((mBoundsRect.left + mBoundsRect.right) / 2)
						- (this.mPaint.measureText(bar.getValueString()) / 2) - 10 * resources
				...
				if (boundLeft < mBoundsRect.left)
					boundLeft = mBoundsRect.left - ((int) padding / 2);// limit
																		// popup
																		// width
																		// to
																		// bar
				...
				if (0 > valueTextSizes
						.indexOfKey(bar.getValueString().length())) {
					// check cache to see if we've done this calculation before
					while (this.mPaint.measureText(bar.getValueString()) > boundRight
							- boundLeft)
						this.mPaint.setTextSize(this.mPaint.getTextSize()
								- (float) 1);
					valueTextSizes.put(bar.getValueString().length(),
							mPaint.getTextSize());
				} else {
					this.mPaint.setTextSize(valueTextSizes.get(bar
							.getValueString().length()));
				}
				canvas.drawText(
						bar.getValueString(),
						(int) (((mBoundsRect.left + mBoundsRect.right) / 2) - (this.mPaint
								.measureText(bar.getValueString())) / 2),
						mBoundsRect.top
								- (mBoundsRect.top - boundTop)
								/ 2f
								+ (float) Math.abs(mTextRect.top
										- mTextRect.bottom) / 2f * 0.7f,
						this.mPaint);
			}
			count++;
		}
