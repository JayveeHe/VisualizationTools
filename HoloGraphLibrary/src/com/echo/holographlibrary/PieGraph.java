...
public class PieGraph extends View {
	...
	// 自行修改
	private String LabelTitle = "";// 图例的第一行
	private String LabelPercentage = "";// 图例的第二行
	...
	public PieGraph(Context context, AttributeSet attrs, int defStyle) {
		...
		mPadding = a
				.getDimensionPixelSize(R.styleable.PieGraph_slicePadding, 0);
	...
	public void onDraw(Canvas canvas) {
		...
		// 计算百分比的字体大小
		int k = getLabelPercentage().length() - 2;
		float p = (float) (2 * innerRadius / Math.sqrt((k * k + 4)));
		labelpaint.setTextSize(p);
		labelpaint.setColor(labelColor);
		canvas.drawText(getLabelPercentage(), (float) (midX - 0.5 * k * p),
				midY, labelpaint);
		// 计算title的字体大小
		k = getLabelTitle().length();
		float t = 0.5f;// title字体的高度占内圆半径的比例
		p = (float) (Math.sqrt((4 - 4 * t * t)) * innerRadius / k);
		if (p > t * innerRadius) {
			p = innerRadius * t;
		}
		labelpaint.setTextSize(p);
		canvas.drawText(getLabelTitle(), (float) (midX - 0.5 * k * p),
				(float) (midY + 0.5 * innerRadius), labelpaint);
	...
	public void setLabelPercentage(float value) {
		float f = value / total * 100f;
		DecimalFormat fnum = new DecimalFormat("##0.0");

		LabelPercentage = fnum.format(f) + "%";
